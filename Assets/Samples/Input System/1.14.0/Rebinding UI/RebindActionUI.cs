...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public TMP_Text rebindPrompt { get => m_RebindText; set => m_RebindText = value; }
        ...
        public GameObject rebindOverlay { get => m_RebindOverlay; set => m_RebindOverlay = value; }
        ...
        //# 하나만 Reset했을 경우, 이미 지정된 키와 중복 여부 검사하여 Swap
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            foreach (var compareAction in action.actionMap.actions)
            {
                ...
                for (int i = 0; i < compareAction.bindings.Count; i++)
                {
                    var binding = compareAction.bindings[i];

                    if (binding.overridePath == newBinding.path)
                    {
                        compareAction.ApplyBindingOverride(i, oldOverridePath);
                    }
                }
            }
        }
        ...
        private bool CheckDuplicatedBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            {
                //# 같은 path 를 가졌다면 continue(설정하려는 키이므로)
                if (newBinding.path == binding.path)
                {
                    continue;
                }

                //# effectivePath : e.g. Keyboard/space
                //# 동일한 입력경로/키가 존재하면 true 반환
            ...
            {
                Debug.Log(action.bindings[0]);
                for (int i = 1; i < bindingIndex; i++)
                {
                    //# bindings의 0번째 인덱스는 Composite 본체 - 키 입력 데이터가 아님
                    //# 예시
                    /*
